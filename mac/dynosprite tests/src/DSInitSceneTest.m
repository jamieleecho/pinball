...
/* The rows are laid out from MenuShowControl and MenuShowMusic in
 * DSInitScene.h, so the indices have to be too: hiding a row closes its gap
 * and everything below it moves up two.  Guarding the expectations with the
 * same switches is what keeps this test honest if a row is ever put back. */
enum DSInitSceneLabelIndices : short {
    ...
    DSInitSceneLabelIndicesCount
};

#if MenuShowControl
static const DSInitSceneControl kExpectedInitialControl = DSInitSceneControlJoystick;
#else
static const DSInitSceneControl kExpectedInitialControl = DSInitSceneControlKeyboard;
#endif
...
- (void)testDidMoveToView {
    XCTAssertEqual(self.target.labels.count, DSInitSceneLabelIndicesCount);
...
#else
    /* No Control row, so the setting is forced rather than read.  That is not
     * cosmetic: SPACE is the emulated joystick's button 0, so leaving the
     * hardware joystick selected would take the start key away with it. */
    XCTAssertEqualObjects(self.target.labels[DSInitSceneLabelIndicesStart].text, @"[Space] to start");
...
- (void)testKeyUpUnknownKey {
    ...
    XCTAssertEqual(self.target.display, DSInitSceneDisplayHigh);
    XCTAssertEqual(self.target.control, kExpectedInitialControl);
    XCTAssertEqual(self.target.sound, DSInitSceneSoundLow);
}
...
- (void)testKeyUpToggleKeyboard {
    ...
    XCTAssertEqual(self.target.control, DSInitSceneControlKeyboard);
#else
    /* The row is compiled out, so C is not a menu key and must do nothing. */
    XCTAssertEqual(self.target.control, DSInitSceneControlKeyboard);
#endif
}
