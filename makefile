...
LOADERBIN = $(GENDISKDIR)/PINBALL.BIN
...
$(GENOBJDIR)/sound%.raw: $(SOUNDDIR)/%.wav
	...
	ffmpeg -nostdin -y -v warning -i $< -acodec pcm_u8 -f u8 -ac 1 -ar $(AUDIORATE) -af aresample=$(AUDIORATE):filter_size=256:cutoff=1.0,volume=0.12 $@
