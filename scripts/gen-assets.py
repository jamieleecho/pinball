 ...
 *
 * The table is traced from art/pinball.png, the nine feet picked out of it by
 * their shape.  It is diced into {gw}x{gh} cells of {playfield.CELL}x{playfield.CELL} pixels, covering the
...
# The score is a seven-segment display, as the original's was: square, chunky
# and unmistakably a machine counting.  Bars two pixels thick fill a 6x11 glyph
# inside the 8x11 cell, which leaves a pixel of margin either side so that
# neighbouring digits do not run into each other.
#
# Eleven rows will not divide symmetrically into three bars and two windows --
# six pixels of bar leaves five -- so the upper window is three rows and the
# lower two, which is the way round that reads best.
SEG_W, SEG_H, SEG_T = 6, 11, 2
SEG_MID = 5  # first row of the middle bar

# Which segments each digit lights, in the usual A..G order: top, upper right,
...
def seg_boxes():
    """Each segment as (x0, y0, x1, y1) in the glyph, corners overlapping.

    The bars are let into each other rather than mitred: a vertical runs the
    whole way from the bar above it to the bar below, so a corner is simply
    where two of them cover the same pixels.  Mitring six-pixel-wide segments
    only loses the corners.
    """
    right, bottom = SEG_W - 1, SEG_H - 1
    t = SEG_T - 1
    return {
        "A": (0, 0, right, t),
        "B": (SEG_W - SEG_T, 0, right, SEG_MID + t),
        "C": (SEG_W - SEG_T, SEG_MID, right, bottom),
        "D": (0, SEG_H - SEG_T, right, bottom),
        "E": (0, SEG_MID, t, bottom),
        "F": (0, 0, t, SEG_MID + t),
        "G": (0, SEG_MID, right, SEG_MID + t),
    }


def digit_pixels(ch):
    """The lit pixels of one digit, as {(x, y)}."""
    boxes = seg_boxes()
    px = set()
    for seg in SEG_DIGITS[ch]:
        x0, y0, x1, y1 = boxes[seg]
        px |= {(x, y) for x in range(x0, x1 + 1) for y in range(y0, y1 + 1)}
    return px
...
# The score read-out, coloured by significance as the original coloured it: the
# three least significant figures cyan, the next three magenta, the most
# significant orange.
#
# The second set is what the game-over flash shows instead of blanking the
# board.  The original XORed the score's pixels, which on its palette swapped
# the background with orange and cyan with magenta; these are those pairs
# written out, because our sixteen colours are not arranged for the trick to
# fall out of the arithmetic.
DIGIT_INKS = (
    ("Lo", TEAL, WHITE),
    ("Mid", MAGENTA, WHITE),
    ("Hi", ORANGE, WHITE),
    ("LoInv", MAGENTA, ORANGE),
    ("MidInv", TEAL, ORANGE),
    ("HiInv", WHITE, ORANGE),
)
...
def draw_digit_sheet(d, img):
    ...
    ox = (DIGIT_W - SEG_W) // 2
    oy = (DIGIT_H - SEG_H) // 2
    for row, (tag, ink, bg) in enumerate(DIGIT_INKS):
        y0 = 2 + row * (DIGIT_H + 4)
        for i in range(10):
            x0 = 2 + i * (DIGIT_W + 4)
            # An opaque tile carrying its own background, so drawing a digit
            # also wipes the one before it and every digit is exactly the same
            # size -- and so that the inverted set brings its orange with it.
            d.rectangle((x0, y0, x0 + DIGIT_W - 1, y0 + DIGIT_H - 1), fill=bg)
            for px, py in digit_pixels(str(i)):
                d.point((x0 + ox + px, y0 + oy + py), fill=ink)
            sprites.append((f"Digit{tag}{i}", x0, y0, False, False))

    # A blank cell.  Read-outs that switch off (the multiplier at 1X) draw this
    # instead of going inactive, because a sprite with no erase code would
    # otherwise stay on screen for ever.
    x0, y0 = 2, 2 + len(DIGIT_INKS) * (DIGIT_H + 4)
    d.rectangle((x0, y0, x0 + DIGIT_W - 1, y0 + DIGIT_H - 1), fill=WHITE)
    sprites.append(("DigitBlank", x0, y0, False, False))
    ...
    for name, ink in (("BallLive", DGREY), ("BallBlank", None)):
        x0 += DIGIT_W + 4
        d.rectangle((x0, y0, x0 + 7, y0 + 7), fill=WHITE)
        if ink is not None:
            d.ellipse((x0 + 1, y0 + 1, x0 + 6, y0 + 6), fill=ink)
            d.point((x0 + 3, y0 + 2), fill=WHITE)
...
def write_sprites():
    ...
    total += write_sprite_group(3, "digit", (128, 128), draw_digit_sheet)
